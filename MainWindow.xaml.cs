...
public partial class MainWindow : Window
{
    ...
    private const string REGISTRY_KEY = @"SOFTWARE\NetflixForDesktop";
    ...
    private const string DISCORD_APP_ID = "1468319689997418517";
    ...
    private async void MainWindow_Loaded(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            // Navigate to Netflix
            Web.Source = new Uri("https://www.netflix.com/");
        }
        catch (Exception ex)
        {
    ...
    private void SetupWebView2()
    {
        ...
        // Monitor navigation and redirect non-Netflix URLs
        Web.CoreWebView2.NavigationStarting += (s2, args) =>
        {
            ...
            {
                args.Cancel = true;
                Web.CoreWebView2.Navigate("https://www.netflix.com/");
            }
        };
    ...
    private void ReloadWebView()
    {
        ...
        if (result == MessageBoxResult.Yes)
        {
            ...
            System.Diagnostics.Process.Start(Environment.ProcessPath ?? "NetflixForDesktop.exe");
            ...
            if (_discordClient?.IsInitialized == true)
            {
                _discordClient.SetPresence(new RichPresence()
                {
                    ...
                    Assets = new Assets()
                    {
                        ...
                        LargeImageText = "Netflix For Desktop"
                    },
    ...
    private void UpdateDiscordPresenceFromUrl(string url)
    {
        try
        {
            ...
            if (path.Contains("/watch/"))
            {
                ...
                UpdateDiscordPresence($"Watching {title}", "Enjoying content");
            }
            else if (path.Contains("/browse"))
            {
                UpdateDiscordPresence("Browsing content", "Looking for something to watch");
            }
            else if (path.Contains("/latest"))
            {
                UpdateDiscordPresence("Checking latest", "Finding new releases");
            }
            else if (path.Contains("/my-list"))
            {
                UpdateDiscordPresence("Managing My List", "Organizing shows to watch");
            }
            else
            {
                UpdateDiscordPresence("Browsing Netflix", "Exploring content");
            }
        }
        catch
        {
            UpdateDiscordPresence("Using Netflix", "Watching content");
        }
    }
